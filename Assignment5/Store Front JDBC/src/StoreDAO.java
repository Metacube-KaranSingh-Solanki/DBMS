...
public class StoreDAO {
	...
	public List<Order> fetchAllOrders(int userId) {
		...
		try {
			...
			String query = "SELECT id, date, amount FROM user_order where user_id = " + userId;
	...
	public int deleteProducts() {
		String mainQuery = "(SELECT temp.pid from(SELECT DISTINCT i.product_id as pid\r\n" + "    FROM item i \r\n"
				+ "    JOIN product p\r\n" + "    ON i.product_id = p.id\r\n" + "    JOIN user_order o\r\n"
				+ "    ON o.id = i.order_id\r\n" + "    JOIN user u\r\n" + "    ON u.id = o.user_id\r\n"
				+ "    WHERE DATEDIFF(curDate(), o.date) < 365 AND u.role='Shopper') temp)";

		String query = "Delete FROM product WHERE id NOT IN " + mainQuery;
		PreparedStatement ps = null;
	...
	public List<Category> categoryList() {
		String query = "SELECT  c.name as Title, count(c.id) as count\r\n" + "FROM category c\r\n"
